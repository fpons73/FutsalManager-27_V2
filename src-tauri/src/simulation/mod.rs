...
pub async fn advance_day(pool: &SqlitePool) -> Result<AdvanceResult, String> {
    ...
    if next.weekday() == Weekday::Mon {
        ...
        let _ = crate::finance::process_stadium_operations(pool).await;
    ...
    let expiring: Vec<(i64,i64,String,String)> = sqlx::query_as("SELECT c.id,c.club_id,c.end_date,p.common_name FROM contracts c JOIN players p ON p.id=c.player_id WHERE c.is_active=1 AND c.loan_parent_id IS NULL AND c.end_date<=? AND c.end_date>=?").bind(&(date+chrono::Duration::days(60)).format("%Y-%m-%d").to_string()).bind(&cur_date).fetch_all(pool).await.map_err(|e| e.to_string())?;
    for (_,club_id,end_date,name) in expiring { sqlx::query("INSERT OR IGNORE INTO inbox_messages(club_id,sender_type,subject,body,date_sent,is_important) VALUES(?,'board',?,?,?,1)").bind(club_id).bind(format!("Contrato próximo a vencer: {}",name)).bind(format!("El contrato de {} termina el {}.",name,end_date)).bind(&cur_date).execute(pool).await.ok(); }
