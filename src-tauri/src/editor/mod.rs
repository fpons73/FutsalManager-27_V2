...
#[derive(Serialize, Deserialize)]
pub struct CompetitionRow { pub id: i64, pub name: String, pub nation: Option<String>, pub nation_id: Option<i64>, pub tier: Option<i64>, pub total_teams: Option<i64>, pub season: String, pub format: String, pub kind: String, pub group_count: i64, pub teams_per_group: i64, pub group_qualifiers: i64, pub knockout_two_legs: i64 }
#[derive(Serialize, Deserialize, Clone)]
...
pub async fn list_competitions(pool: &SqlitePool) -> Result<Vec<CompetitionRow>, String> {
    let rows = sqlx::query_as::<_, (i64, String, Option<i64>, Option<String>, Option<i64>, Option<i64>, String, String, String, i64, i64, i64, i64)>(
    ...
    Ok(rows.into_iter().map(|(id, name, nation_id, nation, tier, total_teams, season, format, kind, group_count, teams_per_group, group_qualifiers, knockout_two_legs)| CompetitionRow { id, name, nation, nation_id, tier, total_teams, season, format, kind, group_count, teams_per_group, group_qualifiers, knockout_two_legs }).collect())
}
...
pub async fn update_competition(pool: &SqlitePool, id: i64, name: String, nation_id: Option<i64>, tier: Option<i64>, teams: i64, season: String, group_count: i64, teams_per_group: i64, group_qualifiers: i64, knockout_two_legs: i64) -> Result<(), String> {
    let kind = if nation_id.is_none() { "national_team" } else { "club" };
    if group_count < 0 || teams_per_group < 0 || group_qualifiers < 0 || knockout_two_legs < 0 || teams < 2 { return Err("Formato de competición no válido".into()); }
    sqlx::query("UPDATE competitions SET name=?, nation_id=?, tier=?, total_teams=?, season=?, kind=?, group_count=?, teams_per_group=?, group_qualifiers=?, knockout_two_legs=? WHERE id=?").bind(name).bind(nation_id).bind(tier).bind(teams).bind(season).bind(kind).bind(group_count).bind(teams_per_group).bind(group_qualifiers).bind(knockout_two_legs).bind(id).execute(pool).await.map_err(|e| e.to_string())?;
    Ok(())
}
