...
pub async fn process_weekly_finances(pool: &SqlitePool) -> Result<(), String> {
    ...
    for (cid,) in clubs {
        ...
        let demand = (reputation / 10 + 35).clamp(10, 100);
        ...
        sqlx::query("UPDATE club_finances SET balance=balance-?+? WHERE club_id=?")
            .bind(weekly_cost).bind(if sale.rows_affected() > 0 { revenue } else { 0.0 }).bind(cid).execute(pool).await.map_err(|e| e.to_string())?;
