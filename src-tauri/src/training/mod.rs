...
pub async fn process_training_week(pool: &SqlitePool, club_id: i64) -> Result<Vec<String>, String> {
    ...
    // La compatibilidad se ajusta lentamente por roles y moral; evitamos cambios bruscos.
    sqlx::query("INSERT OR IGNORE INTO player_relationships(player_id, other_player_id, compatibility) SELECT a.player_id, b.player_id, 50 + CASE WHEN a.squad_role=b.squad_role THEN 5 ELSE 0 END FROM player_roles a CROSS JOIN player_roles b JOIN contracts ca ON ca.player_id=a.player_id AND ca.club_id=? AND ca.is_active=1 JOIN contracts cb ON cb.player_id=b.player_id AND cb.club_id=? AND cb.is_active=1 WHERE a.player_id<>b.player_id").bind(club_id).bind(club_id).execute(pool).await.map_err(|e| e.to_string())?;
    sqlx::query("UPDATE player_relationships SET compatibility=MIN(100,MAX(0,compatibility + CASE WHEN (SELECT ABS(ps1.morale-ps2.morale) FROM player_states ps1 JOIN player_states ps2 ON ps2.player_id=player_relationships.other_player_id WHERE ps1.player_id=player_relationships.player_id) < 15 THEN 1 ELSE -1 END)) WHERE player_id IN (SELECT player_id FROM contracts WHERE club_id=? AND is_active=1)").bind(club_id).execute(pool).await.map_err(|e| e.to_string())?;
    // El liderazgo de los capitanes contagia ligeramente al grupo.
    sqlx::query("UPDATE player_states SET morale=MIN(100,morale+1) WHERE player_id IN (SELECT captain_player_id FROM club_leadership WHERE club_id=? AND captain_player_id IS NOT NULL UNION SELECT vice_captain_player_id FROM club_leadership WHERE club_id=? AND vice_captain_player_id IS NOT NULL)").bind(club_id).bind(club_id).execute(pool).await.map_err(|e| e.to_string())?;
    // Si el editor aún no ha designado capitán, la jerarquía se inicializa con el mayor liderazgo.
    sqlx::query("UPDATE club_leadership SET captain_player_id=(SELECT pr.player_id FROM player_roles pr JOIN contracts c ON c.player_id=pr.player_id WHERE c.club_id=? AND c.is_active=1 ORDER BY pr.leadership DESC, pr.player_id LIMIT 1) WHERE club_id=? AND captain_player_id IS NULL").bind(club_id).bind(club_id).execute(pool).await.map_err(|e| e.to_string())?;
    sqlx::query("UPDATE club_leadership SET vice_captain_player_id=(SELECT pr.player_id FROM player_roles pr JOIN contracts c ON c.player_id=pr.player_id WHERE c.club_id=? AND c.is_active=1 AND pr.player_id<>COALESCE((SELECT captain_player_id FROM club_leadership WHERE club_id=?),-1) ORDER BY pr.leadership DESC, pr.player_id LIMIT 1) WHERE club_id=? AND vice_captain_player_id IS NULL").bind(club_id).bind(club_id).bind(club_id).execute(pool).await.map_err(|e| e.to_string())?;
    sqlx::query("UPDATE player_promises SET status=CASE WHEN promise_type IN ('morale','happiness') AND (SELECT morale FROM player_states WHERE player_id=player_promises.player_id)>=target_value THEN 'fulfilled' WHEN promise_type IN ('morale','happiness') AND (SELECT morale FROM player_states WHERE player_id=player_promises.player_id)<target_value-20 THEN 'failed' ELSE status END, resolved_at=CASE WHEN status IN ('fulfilled','failed') THEN CURRENT_TIMESTAMP ELSE resolved_at END WHERE club_id=? AND status='active'").bind(club_id).execute(pool).await.map_err(|e| e.to_string())?;
