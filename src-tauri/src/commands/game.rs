...
pub async fn get_fixtures(state: State<'_, AppState>, competition_id: i64) -> Result<Vec<FixtureRow>, String> {
    let pool = get_pool(&state).await?;
    // Las partidas creadas antes del generador de calendarios pueden no tener jornadas.
    // Intentamos reparar el calendario de esa competición de forma idempotente antes de leerlo.
    let (scheduled,): (i64,) = sqlx::query_as("SELECT COUNT(*) FROM matches WHERE competition_id=? AND status='scheduled'").bind(competition_id).fetch_one(&pool).await.map_err(|e| e.to_string())?;
    if scheduled == 0 {
        crate::competition::generate_calendars(&pool).await?;
    }
    let rows = sqlx::query_as::<_, FixtureRaw>(
        "SELECT m.id, m.round, COALESCE(ct.leg,1) AS leg, COALESCE(ct.aggregate_home_score,0) AS aggregate_home_score, COALESCE(ct.aggregate_away_score,0) AS aggregate_away_score, m.date, m.home_club_id AS home_id, hc.name AS home_name, hc.short_name AS home_short, m.away_club_id AS away_id, ac.name AS away_name, ac.short_name AS away_short, m.home_score, m.away_score, m.status, comp.competition_type, ct.winner_club_id AS cup_winner_id, COALESCE(ct.went_to_extra_time,0) AS extra, COALESCE(ct.went_to_penalties,0) AS penalties, COALESCE(ct.penalty_home_score,0) AS penalty_home, COALESCE(ct.penalty_away_score,0) AS penalty_away FROM matches m JOIN competitions comp ON comp.id=m.competition_id JOIN clubs hc ON hc.id=m.home_club_id JOIN clubs ac ON ac.id=m.away_club_id LEFT JOIN cup_ties ct ON ct.match_id=m.id WHERE m.competition_id=? ORDER BY m.round, m.date, m.id"
    ).bind(competition_id).fetch_all(&pool).await.map_err(|e| e.to_string())?;
...
pub async fn get_next_fixture(state: State<'_, AppState>, club_id: i64) -> Result<Option<FixtureRow>, String> {
    let pool = get_pool(&state).await?;
    let (scheduled,): (i64,) = sqlx::query_as("SELECT COUNT(*) FROM matches WHERE (home_club_id=? OR away_club_id=?) AND status='scheduled'").bind(club_id).bind(club_id).fetch_one(&pool).await.map_err(|e| e.to_string())?;
    if scheduled == 0 { let _ = crate::competition::generate_calendars(&pool).await; }
