...
pub async fn get_staff_impact(state: State<'_, AppState>) -> Result<crate::training::StaffImpact, String> {
    ...
    let r: (i64,i64,i64,i64,i64) = sqlx::query_as("SELECT COALESCE(MAX(CASE WHEN role='coach' THEN tactical END),0),COALESCE(MAX(CASE WHEN role='assistant' THEN man_management END),0),COALESCE(MAX(CASE WHEN role='scout' THEN judging END),0),COALESCE(MAX(working_youngsters),0),COALESCE(MAX(physio_level),0) FROM staff WHERE club_id=?").bind(c).fetch_one(&pool).await.map_err(|e| e.to_string())?;
    Ok(crate::training::StaffImpact { coach:r.0, assistant:r.1, scout:r.2, youth:r.3, physio:r.4, training_bonus: ((r.0.max(r.1) as f64 / 20.0)*50.0).round() as i64, scouting_bonus: (r.2/10), injury_reduction: (r.4*5).min(95) })
}
