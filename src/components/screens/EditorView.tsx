...
const emptyComp = { id: 0, name: "", nation: "", tier: "", teams: 16, season: "2026/2027", groups: 0, teamsPerGroup: 0, qualifiers: 0, twoLegs: false, tiebreak: "points_goal_difference_goals_for" };
            ...
            <div className="space-y-4">
              <div className="rounded-xl border border-fm-border bg-fm-panel p-3">
                ...
                <div className="flex flex-wrap gap-2">
                  ...
                  <input placeholder="Temporada" value={newComp.season} onChange={(e)=>setNewComp({...newComp,season:e.target.value})} className="w-32 rounded border border-fm-border bg-fm-bg px-2 py-1.5 text-sm" /><input type="number" min="0" placeholder="Grupos" value={newComp.groups} onChange={(e)=>setNewComp({...newComp,groups:Number(e.target.value)})} className="w-20 rounded border border-fm-border bg-fm-bg px-2 py-1.5 text-sm" /><input type="number" min="0" placeholder="Equipos/grupo" value={newComp.teamsPerGroup} onChange={(e)=>setNewComp({...newComp,teamsPerGroup:Number(e.target.value)})} className="w-28 rounded border border-fm-border bg-fm-bg px-2 py-1.5 text-sm" /><input type="number" min="0" placeholder="Clasif." value={newComp.qualifiers} onChange={(e)=>setNewComp({...newComp,qualifiers:Number(e.target.value)})} className="w-20 rounded border border-fm-border bg-fm-bg px-2 py-1.5 text-sm" /><label className="flex items-center gap-1 text-xs text-fm-dim"><input type="checkbox" checked={newComp.twoLegs} onChange={e=>setNewComp({...newComp,twoLegs:e.target.checked})} /> Ida/vuelta</label><select value={newComp.tiebreak} onChange={e=>setNewComp({...newComp,tiebreak:e.target.value})} className="rounded border border-fm-border bg-fm-bg px-2 py-1.5 text-sm"><option value="points_goal_difference_goals_for">Puntos · DG · GF</option><option value="points_head_to_head">Puntos · enfrentamiento directo</option><option value="points_goals_for">Puntos · goles a favor</option></select>
